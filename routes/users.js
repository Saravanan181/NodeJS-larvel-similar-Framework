...
router.post('/login', function(req, res, next) {
    ...
    Users.info(Userinfo,req,res,function(userDetails){
        if(Array.isArray(userDetails) && userDetails.length){
            const token = jwt.sign({ username: userDetails[0].email_id,physican_id:userDetails[0].user_id,  role: 'doctor' }, 'nodeethos576asdas6',{ expiresIn: 60*60*5, algorithm: "HS256" });
